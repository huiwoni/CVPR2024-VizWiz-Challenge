...
def softmax_entropy(x: torch.Tensor) -> torch.Tensor:
    ...
    # x = -(x.softmax(1) * x.log_softmax(dim=1)).sum(dim=1)
    return x

# def mean_softmax_entropy(x:torch.Tensor)->torch.Tensor:
def mean_softmax_entropy(x):
    temprature = 1
    x = x / temprature
...
#############################################################################
# def GEM_T(self, outputs, **kwargs):
def GEM_T(outputs):
    outputs = outputs - outputs.mean(1, keepdim=True).detach()
    T = outputs.std(1, keepdim=True).detach() * 2
    prob = (outputs / T).softmax(1)
...
def GEM_SKD(outputs):
    ...
    original_prob = outputs.softmax(1)
    prob = (outputs / T).softmax(1)

    loss = - ((original_prob.detach() * prob.log()).sum(1) * (T ** 2)).mean()
    return loss

def GEM_Aug(aug_output, outputs, **kwargs):
    outputs = outputs - outputs.mean(1, keepdim=True).detach()
    T = outputs.std(1, keepdim=True).detach() * 2
    loss = - ((aug_output.softmax(1).detach() * (outputs / T).softmax(1).log()).sum(1) * (T ** 2)).mean()
    return loss
...
def forward_and_adapt(self, x, model, optimizer):
    ...
    # aug_x = x
    # N, C, H, W = aug_x.shape
    # aug_x = aug_x.reshape(N, C, H, W)
    # aug_x = aug_x.reshape(N * 1, C, H, W)
    # aug_output = model(aug_x)
    # aug_output = model(aug_x).view(N, 1, -1).mean(1)
    ...
    # adapt gem-t
    loss = GEM_T(outputs)
    # loss = GEM_SKD(outputs)
    # loss = GEM_Aug(aug_output,outputs)

    # adapt ( 원본 )
    # loss = softmax_entropy(loss).mean(0)
    # loss = softmax_entropy(outputs).mean(0)
    loss.backward()
    optimizer.step()
    optimizer.zero_grad()

    loss_file_path = "/mnt/HDD2/HW_2/Benchmark-TTA/SH/loss/test.txt"
