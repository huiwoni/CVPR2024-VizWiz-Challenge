...
from ..utils.utils import split_up_model, get_output, lr_scheduler, Entropy, cal_acc, create_shot_predict
...
def train_target_ttba(cfg, domain, severity=5, type='eval'):
    ...
    for epoch in range(cfg.TEST.EPOCH):
        ...
        for inputs_test, _, tar_idx in loader:
            ...
            logger.info(f'loss {classifier_loss}')
            optimizer.zero_grad()
...
############################################################################################################ func start
    ...
    predictions = np.concatenate(prediction, 0)
    testdata_list = loader.dataset.dataset.samples

    return predictions, testdata_list
...
############################################################################################################ func start
            ...
            model.eval()
            prediction.append(outputs_test.detach().cpu().numpy())
            model.train()
...
def train_target_ttda(cfg, domain, severity=5, type='eval'):
 ...
 ####################################################################################################################### func start
        ...
        if epoch == cfg.TEST.EPOCH - 1:
            model.eval()
            pred, testdata_list = create_shot_predict(loader, model)
