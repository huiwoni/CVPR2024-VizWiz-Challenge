...
def load_imagenet_c(root, corruption, transforms, level=1, batch_size=64, workers=4, ckpt=None):
...
def load_692_data(root, corruption, transforms, level=5, batch_size=64, workers=4, ckpt=None):
    assert os.path.exists(root), f'Path {root} does not exist'
    assert corruption in ['original'], f'Unknown corruption: {corruption}'
    ...
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)
    ckpt_path = os.path.join(ckpt_dir, 'list_challenge.pickle')
    if not os.path.exists(ckpt_path):
        idx = torch.randperm(len(teset))
        idx = [i.item() for i in idx]
        with open(ckpt_path, 'wb') as f:
    ...
    return teset, teloader
def load_challenge_1_data(root, corruption, transforms, level=5, batch_size=64, workers=4, ckpt=None):
    assert os.path.exists(root), f'Path {root} does not exist'
    assert corruption in ['original'], f'Unknown corruption: {corruption}'
    ...
    ckpt_dir = os.path.join(ckpt, 'challenge_1')
    if not os.path.exists(ckpt_dir):
        os.makedirs(ckpt_dir)
    ckpt_path = os.path.join(ckpt_dir, 'list.pickle')
    if not os.path.exists(ckpt_path):
...
def load_dataset(dataset, root, batch_size=64, workers=4, split='train', adaptation=None, domain=None,
    ...
    elif dataset == 'challenge_test_debug':
        return load_challenge_1_data(root=os.path.join(root, 'challenge_test_debug'), batch_size=batch_size, corruption=domain,
                               level=level, workers=workers,
                               transforms=transforms, ckpt=ckpt)

    elif dataset == '692':
        return load_692_data(root=os.path.join(root), batch_size=batch_size, corruption=domain,
                               level=level, workers=workers,
                               transforms=transforms, ckpt=ckpt)
    ####################################################################################################################
