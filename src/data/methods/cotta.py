...
class CoTTA(nn.Module):
    ...
    def __init__(self, model, optimizer, mt_alpha, rst_m, ap, dataset_name, steps=1, episodic=False, num_aug=32, adaptation_type='otta'):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        if self.adaptation_type == 'ttba':
            ...
            for _ in range(self.steps):
                _ = self.forward_and_adapt_ttba(x, self.optimizer)
...
######################################################################################################################################### forward end
    ...
    @torch.enable_grad()  # ensure grads in possible no grad context for testing
    def forward_and_adapt_ttba(self, x, optimizer):
        outputs = self.model(x)
        self.model_ema.train()
        # Teacher Prediction
        anchor_prob = torch.nn.functional.softmax(self.model_anchor(x), dim=1).max(1)[0]
