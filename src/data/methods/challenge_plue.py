...
def soft_k_nearest_neighbors(features, features_bank, probs_bank, num_neighbors):
    ...
    # 각 샘플에서 가장 높은 확률을 갖는 클래스의 값을 선택
    ...
    # 각 샘플과 각 이웃에 대해 가장 높은 확률을 갖는 클래스의 값을 선택
    _, pred_labels_hard = pred_probs_all.max(dim=1)[0].max(dim=1)

    return pred_labels, pred_probs, pred_labels_all, pred_labels_hard, pred_probs
...
def refine_predictions(
    ...
    pred_labels, probs, pred_labels_all, pred_labels_hard, pred = soft_k_nearest_neighbors(
        features, feature_bank, probs_bank, num_neighbors
    )

    return pred_labels, probs, pred_labels_all, pred_labels_hard, pred
...
def eval_and_label_dataset(epoch, model, loader, cfg):
    ...
    pred_labels, _, _, _, pred = refine_predictions(features, probs, banks, cfg.PLUE.NUM_NEIGHBORS)

    acc = 100. * accuracy_score(gt_labels.to('cpu'), pred_labels.to('cpu'))

    return acc, banks, gt_labels, pred_labels, loader.dataset.dataset.samples, pred

def train_ttba(epoch, net, moco_model, optimizer, trainloader, banks, cfg, CE):
    ...
    moco_model_state = deepcopy(moco_model.state_dict())
    ...
    for batch_idx, batch in enumerate(tqdm(trainloader)):
        ...
        moco_model.load_state_dict(moco_model_state, strict=True)
        ...
        if cfg.PLUE.LABEL_REFINEMENT:
            with torch.no_grad():
                ...
                pseudo_labels_w, probs_w, _, _, _ = refine_predictions(feats_w, probs_w, banks, cfg.PLUE.NUM_NEIGHBORS)
        else:
            probs_w = F.softmax(logits_w, dim=1)
        ...
        _, _, _, _, pred = refine_predictions(feats_w, probs_w, banks, cfg.PLUE.NUM_NEIGHBORS)
    ...
    return prediction, trainloader.dataset.dataset.samples

    ####################################################################################################################

def train_otta(epoch, net, moco_model, optimizer, trainloader, banks, cfg, CE):
    loss = 0
    acc = 0
    ...
    for batch_idx, batch in enumerate(tqdm(trainloader)):
        ...
        if cfg.PLUE.LABEL_REFINEMENT:
            with torch.no_grad():
                probs_w = F.softmax(logits_w, dim=1)
                pseudo_labels_w, probs_w, _, _, pred = refine_predictions(feats_w, probs_w, banks, cfg.PLUE.NUM_NEIGHBORS)
        else:
            probs_w = F.softmax(logits_w, dim=1)
            pseudo_labels_w = probs_w.max(1)[1]
...
def train_ttda(epoch, net, moco_model, optimizer, trainloader, banks, cfg, CE):
    loss = 0
    acc = 0
...
def train_target_ttba(cfg, domain, severity=5, type='eval'):
    dataset, loader = load_dataset_idx(cfg.CORRUPTION.DATASET, '/mnt/HDD2/HW_2/Benchmark-TTA/datasets',
                                       cfg.TEST.BATCH_SIZE,
    ...
    acc, banks, _, _, _, _ = eval_and_label_dataset(0, moco_model, loader, cfg)
    ...
    for epoch in range(cfg.TEST.EPOCH + 1):
        ...
        pred, test_data_list= train_ttba(epoch, model, moco_model, optimizer, loader, banks, cfg, CE)
        torch.cuda.empty_cache()

        ################################################################################################################ func end

    return pred, test_data_list

def train_target_otta(cfg, domain, severity=5, type='eval'):
    dataset, loader = load_dataset_idx(cfg.CORRUPTION.DATASET, '/mnt/HDD2/HW_2/Benchmark-TTA/datasets',
                                       cfg.TEST.BATCH_SIZE,
                                       split='all', domain=domain, level=severity,
                                       adaptation=cfg.MODEL.ADAPTATION,
    ...
    for epoch in range(cfg.TEST.EPOCH + 1):
        ...
        ################################################################################################################ if episodic
        pred, test_data_list= train_otta(epoch, model, moco_model, optimizer, loader, banks, cfg, CE)  # train net1
        torch.cuda.empty_cache()
        ################################################################################################################

        ################################################################################################################ if not episodic
        # train(epoch, model, moco_model, optimizer, loader, banks, cfg, CE)
        # torch.cuda.empty_cache()
        # acc, banks, gt_labels, pred_labels, test_data_list, pred = eval_and_label_dataset(epoch, moco_model, loader, cfg)
...
def train_target_ttda(cfg, domain, severity=5, type='eval'):
    dataset, loader = load_dataset_idx(cfg.CORRUPTION.DATASET, '/mnt/HDD2/HW_2/Benchmark-TTA/datasets',
                                       cfg.TEST.BATCH_SIZE,
                                       split='all', domain=domain, level=severity,
                                       adaptation=cfg.MODEL.ADAPTATION,
    ...
    for epoch in range(cfg.TEST.EPOCH + 1):
        ...
        ################################################################################################################ if not episodic
        train_ttda(epoch, model, moco_model, optimizer, loader, banks, cfg, CE)
