...
from src.utils import create_submit_file, get_args, create_submit_file_for_new_idea, create_submit_file_for_new_idea_lame
...
def evaluate(cfg):
    ...
    if cfg.MODEL.ADAPTATION == "source":  # BN--0
        ...
        model = setup_vida(base_model, cfg)
    elif cfg.MODEL.ADAPTATION == "sar_para":
        model = setup_sar_para(base_model, cfg, num_classes)
    elif cfg.MODEL.ADAPTATION == 'tent_para':
        model, param_names = setup_tent_para(base_model, cfg)
    elif cfg.MODEL.ADAPTATION == 'parallel_psedo':
